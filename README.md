...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.21523052.svg)](https://doi.org/10.5281/zenodo.21523052)
...
**Note: `T2_us=None` on this entry** — topological qubits don't have a conventional measured dephasing time to report, and no value was invented to fill the gap. Code that reads `T2_us` directly should check `is not None` before operating on it (the package's own internal API already does this everywhere).
